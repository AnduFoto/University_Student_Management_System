...
import axios from 'axios';
import { 
  FaUser, FaGraduationCap, FaChartLine, FaBook, 
  FaSyncAlt, FaCalendarAlt, FaChalkboardTeacher,
...
const StudentDashboard = () => {
  ...
  const fetchInstructorDetails = async (instructorId) => {
    if (!instructorId) return null;
    const endpoints = [`teachers/${instructorId}/`, `teachers/teachers/${instructorId}/`, `teacher/${instructorId}/` ];
    for (const endpoint of endpoints) {
      try {
        const response = await safeApiCall(endpoint);
  ...
  const fetchStudentData = async () => {
    try {
      ...
      const [studentResponse, coursesResponse, allCourses, gradesResponse] = await Promise.all([
        safeApiCall(`students/${username}/`),
        safeApiCall(`students/${username}/courses/`),
        safeApiCall('courses/courses/'),
        safeApiCall(`students/${username}/grades/`)
      ]);

      setStudentData(studentResponse);
      const studentCoursesData = coursesResponse?.courses || [];
      const gradesData = gradesResponse?.grades || [];
      setGrades(gradesData);

      const detailedCourses = studentCoursesData.map(sc => ({
        ...sc,
        ...(allCourses?.find(c => c.course_id === sc.course_id) || {})
      }));
      setCourseDetails(detailedCourses);

      const instructorsMap = {};
      await Promise.all(detailedCourses.map(async (course) => {
        if (course.instructor) {
          instructorsMap[course.course_id] = await fetchInstructorDetails(course.instructor);
        }
      }));
      setInstructors(instructorsMap);
    } catch (err) {
  ...
  const calculateGPA = () => {
    if (!grades.length) return '0.00';
    const totalPoints = grades.reduce((sum, g) => sum + ((Number(g.final) || 0) / 25), 0); // Logic adapted for 4.0 scale
    return (totalPoints / grades.length).toFixed(2);
  };
  ...
  const toggleSemester = (sem) => setExpandedSemesters(prev => ({ ...prev, [sem]: !prev[sem] }));

  const coursesBySemester = courseDetails.reduce((acc, course) => {
    const key = `${course.course_taken_year} - Sem ${course.course_taken_semester}`;
    if (!acc[key]) acc[key] = [];
    acc[key].push(course);
    return acc;
  }, {});
  ...
  return (
    <div className="animate-in fade-in duration-700 max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
        <div>
          <h2 className="text-2xl font-black text-gray-800 uppercase tracking-tighter">
            Academic <span className="text-orange-500">Records</span>
          </h2>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
            <FaShieldAlt className="text-green-500" /> Official Transcript View
          </p>
        </div>
        <button 
          onClick={fetchStudentData}
          className="flex items-center justify-center gap-2 bg-[#343a40] text-white px-5 py-2.5 rounded-lg font-bold text-[10px] uppercase tracking-widest hover:bg-orange-600 transition-all active:scale-95 shadow-md"
        >
          <FaSyncAlt /> Sync Data
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Cumulative GPA", val: `${calculateGPA()} / 4.0`, icon: <FaChartLine />, color: "border-orange-500" },
          { label: "Total Courses", val: courseDetails.length, icon: <FaBook />, color: "border-blue-500" },
          
        ].map((stat, i) => (
          <div key={i} className={`bg-white p-6 rounded-xl border-t-4 ${stat.color} shadow-sm flex items-center justify-between`}>
            <div>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">{stat.label}</p>
              <h3 className="text-2xl font-black text-gray-800">{stat.val}</h3>
            </div>
            <div className="text-gray-200 text-3xl">{stat.icon}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            <div className="bg-[#343a40] p-4 text-center">
               <div className="w-20 h-20 bg-orange-500 rounded-full mx-auto flex items-center justify-center border-4 border-white/10 shadow-lg mb-2">
                 <FaUser className="text-white text-3xl" />
               </div>
               <h4 className="text-white font-bold uppercase tracking-tight">{studentData?.user_details?.firstName}</h4>
               <p className="text-orange-500 text-[10px] font-black uppercase tracking-widest">{studentData?.userId}</p>
            </div>
            <div className="p-6 space-y-4">
              <div className="flex flex-col">
                <span className="text-[9px] font-bold text-gray-400 uppercase">Department</span>
                <span className="text-sm font-bold text-gray-700">{studentData?.department_details?.department_name}</span>
              </div>
              <div className="flex flex-col pt-3 border-t border-gray-50">
                <span className="text-[9px] font-bold text-gray-400 uppercase">Institutional Email</span>
                <span className="text-sm font-medium text-gray-600 truncate">{studentData?.user_details?.email}</span>
              </div>
              <div className="flex flex-col pt-3 border-t border-gray-50">
                <span className="text-[9px] font-bold text-gray-400 uppercase">Contact Number</span>
                <span className="text-sm font-medium text-gray-600">{studentData?.user_details?.phoneNumber || 'Not Provided'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 space-y-4">
          {Object.entries(coursesBySemester).map(([semester, semesterCourses]) => (
            <div key={semester} className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              <button
                onClick={() => toggleSemester(semester)}
                className="w-full px-6 py-4 flex justify-between items-center bg-gray-50 hover:bg-white transition-colors"
              >
                <div className="flex items-center gap-3">
                  <FaGraduationCap className="text-orange-500" />
                  <span className="font-black text-gray-700 uppercase text-xs tracking-wider">{semester}</span>
                  <span className="bg-gray-200 text-[9px] font-bold px-2 py-0.5 rounded-full text-gray-500">
                    {semesterCourses.length} UNITS
                  </span>
                </div>
                {expandedSemesters[semester] ? <FaChevronUp className="text-gray-400" /> : <FaChevronDown className="text-gray-400" />}
              </button>

              {expandedSemesters[semester] && (
                <div className="p-4 space-y-4 border-t border-gray-100 animate-in slide-in-from-top-2">
                  {semesterCourses.map(course => {
                    const gradeObj = grades.find(g => g.course_id === course.course_id);
                    const instructor = instructors[course.course_id];
                    return (
                      <div key={course.course_id} className="group border border-gray-100 rounded-xl p-4 hover:border-orange-200 hover:bg-orange-50/30 transition-all">
                        <div className="flex justify-between items-start mb-4">
                          <div>
                            <h5 className="font-black text-gray-800 uppercase text-sm tracking-tighter group-hover:text-orange-600 transition-colors">
                              {course.course_name}
                            </h5>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest">
                              {course.course_id} • {course.course_credit} Credits
                            </p>
                          </div>
                          {gradeObj && (
                            <div className="text-right">
                              <span className={`text-xl font-black ${getGradeColor(gradeObj.final)}`}>
                                {gradeObj.final}%
                              </span>
                              <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Final Grade</p>
                            </div>
                          )}
                        </div>

                        {/* Instructor Detail */}
                        <div className="flex items-center gap-3 bg-white border border-gray-100 p-2.5 rounded-lg">
                          <div className="w-8 h-8 bg-gray-100 rounded-full flex items-center justify-center text-gray-400">
                             <FaChalkboardTeacher />
                          </div>
                          <div>
                            <p className="text-[10px] font-black text-gray-800 uppercase leading-none">
                              {instructor ? `${instructor.user_first_name} ${instructor.user_father_name}` : "Not Assigned"}
                            </p>
                            <p className="text-[9px] text-gray-400 font-bold uppercase mt-1">Course Lead</p>
                          </div>
                        </div>
                      </div>
                    );
          ...
          {courseDetails.length === 0 && (
            <div className="text-center py-20 bg-white rounded-xl border border-dashed border-gray-300">
               <FaBook className="text-gray-200 text-5xl mx-auto mb-4" />
               <p className="text-gray-400 font-bold uppercase text-xs tracking-widest">No Course History Found</p>
            </div>
          )}
        </div>
      </div>
