...
//             Inatructor <br className="sm:hidden" /> Dashboard
//           </span>
//         </div>
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white border-b border-slate-200 shadow-sm h-16 select-none">
      <div className="flex items-center justify-between h-full px-4 md:px-8">
        ...
        {/* Left Section: Branding matching Admin structure */}
        <div className="flex items-center gap-4 w-1/4">
          <div className="bg-orange-600 p-2 rounded-lg text-white hidden sm:block">
            <FaUserCircle className="text-xl" />
          </div>
          <span className="text-xl font-bold text-slate-800 tracking-tight hidden lg:block">
            USMS<span className="text-orange-600">Instructor</span>
          </span>
        </div>

        {/* Center Section: Main Dashboard Heading */}
        <div className="flex items-center justify-center w-1/3 text-center">
          <span className="text-base sm:text-lg font-black tracking-wider text-slate-800 uppercase hidden md:inline-block">
            Instructor <span className="text-orange-500">Dashboard</span>
          </span>
        </div>

        {/* Right Section: Profile Component Configuration */}
        <div className="flex items-center justify-end w-1/4 gap-3 md:gap-6">
          ...
          <div className="relative">
            ...
            >
              {/* Profile Avatar Frame with Active Status Indicator */}
              <div className="relative">
                {user?.picture ? (
                  <img
                    src={`http://127.0.0.1:8000${user.picture}`}
            ...
            {/* Exactly Mirrored Modern Dropdown Panel */}
            {dropdownOpen && (
              <div
                ref={dropdownRef}
              ...
              >
                <div className="px-4 py-3 border-b border-slate-50">
                  ...
                  <p className="text-sm text-slate-600 truncate mt-1">{user?.email || "instructor@example.com"}</p>
                </div>
