...
//      { name: "User Management", label: "UserName", link: "username-print", icon: <FaUsersCog /> },
//     { name: "Password Reset", label: "Password Reset", link: "password-reset", icon: <FaKey /> },
//   ];

//   // Automatically hide sidebar when clicking on mobile
...
//                 alt={user.name || "User"}
//                 className="w-24 h-24 object-cover border-2 border-black shadow-lg mt-6"
//               />
//             ) : (
//               <FaUserCircle className="text-3xl sm:text-4xl text-black mt-6" />
//             )}
...
//               return (
//                 <Link key={tab.name} to={tab.link} onClick={() => window.innerWidth < 768 && handleHideSidebar()}>
//                   <div
//                     className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer mb-2 transition-all duration-200
//                       ${isActive ? "bg-white text-orange-600 shadow-md" : "hover:bg-orange-400 hover:shadow-md"}
//                     `}
//                   >
...
//         {/* Sidebar toggle arrow outside when hidden */}
//         {!showSidebar && (
//           <div className="fixed top-6 left-2 z-30 md:left-4">
//             <button
//               onClick={() => setShowSidebar(true)}
//               className="bg-orange-500 top-20 mt-16 text-white rounded-full w-10 h-10 flex items-center justify-center hover:bg-orange-600 shadow-lg transition"
//               title="Show sidebar"
//             >
//               →
//             </button>
...
//     { name: "User Registration", label: "User Registration", link: "userregistration", icon: <FaUserPlus /> },
//     { name: "User Management", label: "User Management", link: "usermanagement", icon: <FaUsersCog /> },
//     { name: "UserName Print", label: "UserName", link: "username-print", icon: <FaUsersCog /> },
//     { name: "Password Reset", label: "Password Reset", link: "password-reset", icon: <FaKey /> },
//   ];
...
//       {/* Container Adjusted for Navbar Height */}
//       <div className="flex h-[calc(100vh-4rem)] mt-16 relative">
...
//                   <FaUniversity />
//                 </div>
//                 <span className="text-lg font-light text-white tracking-wide">Admin Panel</span>
//              </div>
...
//           {/* Navigation Menu - Fixed Scrolling here */}
//           <nav className="flex-1 overflow-y-auto mt-2 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
//             <p className="px-4 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest">Main Navigation</p>
//             <div className="pb-20"> {/* Added padding bottom to ensure last items aren't hidden */}
//               {tabsside.map((tab) => {
//                 const isActive = location.pathname.includes(tab.link);
//                 return (
//                   <Link 
...
//                         ${isActive 
//                           ? "bg-[#494e54] text-white border-orange-500" 
...
//         {/* Main Content Area */}
//         <div
//           className={`flex-1 bg-white h-full overflow-y-auto transition-all duration-300
//             ${showSidebar ? "md:ml-64" : "ml-0"}
//           `}
//         >
//           {/* Breadcrumb / Title Bar Header */}
//           <div className="bg-[#f8f9fa] border-b px-6 py-4 flex items-center justify-between">
//              <h1 className="text-xl font-semibold text-gray-700">Dashboard</h1>
//              <div className="text-xs text-gray-400">
//                 Home &gt; Admin &gt; <span className="text-orange-500">Current Page</span>
//              </div>
//           </div>
...
import { Link, Outlet, useLocation } from "react-router-dom";
import { 
  FaUserCircle, FaUsers, FaPlusCircle, FaBook, FaUniversity,
  FaSchool, FaLayerGroup, FaClipboardList, FaUserPlus,
  FaUsersCog, FaKey, FaBars, FaChevronLeft, FaCalendarAlt, FaClock, FaShieldAlt
} from "react-icons/fa";
import AdminNavbar from './AdminNavbar';
...
const Registeraldashboard = () => {
  ...
  // IMPROVED LOGIC: If the URL is JUST the dashboard path, show the welcome content.
  // This ensures the white space is filled immediately after login.
      ...
      <div className="flex h-[calc(100vh-4rem)] mt-16 relative">
        ...
        >
          ...
          <div className="flex-shrink-0 flex items-center gap-3 p-4 border-b border-gray-700">
            ...
            <div className="overflow-hidden">
              <p className="text-sm font-medium text-white truncate">
                {user?.firstName || "Admin"} {user?.fatherName || ""}
              </p>
              <div className="flex items-center gap-1">
                <span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>
                <span className="text-[10px] uppercase text-gray-400">Online</span>
        ...
        <div className={`flex-1 bg-[#f4f6f9] h-full overflow-y-auto transition-all duration-300 ${showSidebar ? "md:ml-64" : "ml-0"}`}>
          ...
          <div className="bg-white border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
             <div className="flex flex-col">
                <h1 className="text-xl font-black text-gray-800">
                  Welcome, <span className="text-orange-500">{user?.firstName || "Andualem"}</span>
                </h1>
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1">
                  <FaShieldAlt className="text-green-600" /> Manage your university securely
                </p>
             </div>
          ...
          <div className="p-4 sm:p-6 md:p-8">
            {/* THIS IS THE SECTION THAT WILL FILL THE WHITE SPACE */}
            {isLandingPage ? (
                ...
                <div className="bg-white rounded-2xl p-8 shadow-sm border border-gray-200 relative overflow-hidden">
                  <div className="relative z-10">
                    ...
                    <p className="text-gray-500 text-sm max-w-md leading-relaxed">
                      Select a module from the sidebar to manage students, courses, or departments. 
                      Everything you need for university administration is at your fingertips.
                    </p>
                    <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div className="p-4 bg-orange-50 rounded-xl border border-orange-100">
                        <div className="text-orange-600 font-black text-lg">Active</div>
                        <div className="text-[10px] text-orange-400 uppercase font-bold">System Status</div>
                      </div>
                      <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                        <div className="text-blue-600 font-black text-lg">Secure</div>
                        <div className="text-[10px] text-blue-400 uppercase font-bold">Data Link</div>
                      </div>
                      <div className="p-4 bg-green-50 rounded-xl border border-green-100">
                        <div className="text-green-600 font-black text-lg">Verified</div>
                        <div className="text-[10px] text-green-400 uppercase font-bold">Identity</div>
                      </div>
                    </div>
                  </div>
                  {/* Faded Background Logo */}
                  <FaUniversity className="absolute -bottom-10 -right-10 text-gray-100 text-[200px] rotate-12" />
                </div>

                {/* Quick Shortcuts */}
                <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                  <Link to="userregistration" className="group bg-white p-6 rounded-2xl shadow-sm border border-gray-200 hover:border-orange-500 transition-all">
                    <h3 className="font-bold text-gray-800 flex items-center gap-2">
                      <FaUserPlus className="text-orange-500" /> New User Registration
                    </h3>
                    <p className="text-xs text-gray-400 mt-1">Register new faculty or staff members immediately.</p>
                  </Link>
                  <Link to="courseslist" className="group bg-white p-6 rounded-2xl shadow-sm border border-gray-200 hover:border-blue-500 transition-all">
                    <h3 className="font-bold text-gray-800 flex items-center gap-2">
                      <FaBook className="text-blue-500" /> View Course List
                    </h3>
                    <p className="text-xs text-gray-400 mt-1">Review all active academic courses and catalogs.</p>
                  </Link>
                </div>
              </div>
            ) : (
