...
//             Department <br className="sm:hidden" /> Dashboard
//           </span>
//         </div>
...
export default function Navbar() {
  ...
  // Load user from localStorage on mount (Preserved exactly)
  useEffect(() => {
    const storedUser = localStorage.getItem("user");
    if (storedUser) setUser(JSON.parse(storedUser));
  }, []);

  // Multi-Token Clear Logout (Preserved exactly)
  const logout = () => {
    localStorage.removeItem("access");
    localStorage.removeItem("refresh");
    localStorage.removeItem("role");
  ...
  // Close dropdown when clicking outside (Preserved exactly)
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (
        dropdownRef.current &&
  ...
  // Dynamic Position Bounds Sync (Preserved exactly)
  useEffect(() => {
    if (dropdownOpen && buttonRef.current) {
      const rect = buttonRef.current.getBoundingClientRect();
      setDropdownStyles({
        position: "fixed",
        top: rect.bottom + 8, // Synced to structural standard
        left: rect.left - 40, // Offset to neatly accommodate wider layout cards
        width: "14rem",       // Replaced explicit button matching with premium standard card width
        zIndex: 9999,
      });
    }
  }, [dropdownOpen]);
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-50 bg-white border-b border-slate-200 shadow-sm h-16 select-none">
      <div className="flex items-center justify-between h-full px-4 md:px-8">
        ...
        {/* Left Section: Matched Branding Block */}
        <div className="flex items-center gap-4 w-1/4">
          ...
          <span className="text-xl font-bold text-slate-800 tracking-tight hidden lg:block">
            USMS<span className="text-indigo-600">Dept</span>
          </span>
        </div>

        {/* Center Section: Matched Global Search Bar Layout */}
        ...
        <div className="flex items-center justify-end w-1/4 gap-3 md:gap-6">
          ...
          <div className="relative">
            ...
            >
              ...
              <FaChevronDown className={`text-slate-400 text-xs transition-transform ${dropdownOpen ? "rotate-180" : ""}`} />
            </button>

            {/* Matched Premium Dropdown Menu Layout */}
            {dropdownOpen && (
              <div
                ref={dropdownRef}
                style={dropdownStyles}
                className="bg-white rounded-2xl shadow-xl border border-slate-100 py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200"
              >
                <div className="px-4 py-3 border-b border-slate-50">
                  <p className="text-xs font-semibold text-slate-400 uppercase">Account Context</p>
                  <p className="text-sm text-slate-600 truncate mt-1">{user?.email || "dept@university.edu"}</p>
                </div>
