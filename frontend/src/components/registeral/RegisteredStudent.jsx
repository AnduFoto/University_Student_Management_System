...
const StudentManagement = () => {
  ...
  // --- MEMOIZED DATA (Preserved) ---
  const availableBatches = useMemo(() => {
    if (!activeDepartment) return [];
    const deptStudents = students.filter(s => s.department_id === activeDepartment.department_id);
    const batchSet = new Set();
    deptStudents.forEach((s) => {
      const user = users.find(u => u.username === s.username);
      if (user && user.batch) batchSet.add(user.batch);
    });
    return Array.from(batchSet).sort();
  }, [students, users, activeDepartment]);
  ...
  const availableSemesters = useMemo(() => {
    ...
    const deptBatchStudents = students.filter((s) => {
      const user = users.find(u => u.username === s.username);
      return s.department_id === activeDepartment.department_id && user && user.batch === selectedBatch;
    });
    const semesterSet = new Set();
    deptBatchStudents.forEach((s) => Object.keys(s.semesters || {}).forEach((sem) => semesterSet.add(sem)));
    return Array.from(semesterSet).sort();
  }, [students, users, activeDepartment, selectedBatch]);
  ...
  const filteredStudents = useMemo(() => {
    ...
    students.forEach((s) => {
      const user = users.find(u => u.username === s.username);
      const matchesDeptBatch = s.department_id === activeDepartment.department_id && user && user.batch === selectedBatch;
      const hasSelectedSemester = s.semesters && s.semesters[selectedSemester];
      if (matchesDeptBatch && hasSelectedSemester) studentMap.set(s.username, s);
    });
    const uniqueStudents = Array.from(studentMap.values());
    if (!searchTerm) return uniqueStudents;
    return uniqueStudents.filter((s) => {
      const fields = [s.username, s.user_details?.firstName, s.user_details?.fatherName];
      return fields.filter(Boolean).some((f) => f.toLowerCase().includes(searchTerm.toLowerCase()));
    });
  }, [students, users, activeDepartment, selectedBatch, selectedSemester, searchTerm]);

  const filteredDepartments = useMemo(() => {
    if (!deptSearch) return departments;
    return departments.filter((d) => d.department_name.toLowerCase().includes(deptSearch.toLowerCase()));
  }, [departments, deptSearch]);

  const csvData = filteredStudents.map((s) => ({
    Username: s.username,
    Name: `${s.user_details?.firstName || ""} ${s.user_details?.fatherName || ""}`,
    Batch: selectedBatch,
    Department: s.department_details?.department_name,
    Semester: selectedSemester,
    Courses: s.semesters[selectedSemester].map((c) => `${c.course_name} (${c.course_credit}cr)`).join("; "),
  }));
  ...
  const downloadSlip = (student, semester) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.text("Academic Registration Slip", 74, 20, { align: "center" });
    doc.setFontSize(8);
    doc.text("Infolink University College", 74, 25, { align: "center" });
    doc.setDrawColor(79, 70, 229); // Indigo line
    doc.line(15, 30, 133, 30);
    
    doc.setFontSize(10);
    doc.setFont("helvetica", "normal");
    doc.text(`Name: ${student.user_details?.firstName || ""} ${student.user_details?.fatherName || ""}`, 15, 40);
    doc.text(`ID: ${student.username}`, 15, 46);
    doc.text(`Dept: ${student.department_details?.department_name}`, 15, 52);
    doc.text(`Period: ${semester}`, 15, 58);

    const courseData = student.semesters[semester].map((c, i) => [i + 1, c.course_id, c.course_name, c.course_credit]);
    autoTable(doc, {
      startY: 65,
      head: [["#", "Code", "Course Name", "Cr"]],
      body: courseData,
      theme: "grid",
      headStyles: { fillColor: [30, 41, 59] }, // Slate head
      styles: { fontSize: 8 }
    });
    doc.save(`${student.username}_slip.pdf`);
  };

  return (
    <div className="min-h-screen bg-[#F8FAFC] pb-20">
      
      {/* 1. INITIAL DEPARTMENT SELECTION */}
      {!isLoading && !activeDepartment && (
        <div className="max-w-7xl mx-auto px-8 pt-10 animate-fadeIn">
          <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6">
            <div>
              <h1 className="text-4xl font-black text-slate-900 tracking-tighter italic">Select <span className="text-indigo-600">Department</span></h1>
              <p className="text-slate-500 font-medium">Choose a department to manage student records</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDepartments.map((dept) => (
              <div 
                key={dept.department_id}
                onClick={() => setActiveDepartment(dept)}
                className="group bg-white border border-slate-200 p-8 rounded-[2rem] cursor-pointer hover:border-indigo-500 hover:shadow-2xl hover:-translate-y-1 transition-all"
              >
                <div className="flex justify-between items-start mb-6">
                  <div className="p-4 bg-slate-900 rounded-2xl group-hover:bg-indigo-600 transition-colors">
                    <BuildingLibraryIcon className="h-8 w-8 text-indigo-400 group-hover:text-white" />
                  </div>
                  <ChevronRightIcon className="h-6 w-6 text-slate-300 group-hover:text-indigo-500" />
                </div>
                <h3 className="font-black text-slate-900 text-xl tracking-tight leading-tight">{dept.department_name}</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">View Batches</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* 2. BATCH SELECTION */}
      {!isLoading && activeDepartment && !selectedBatch && (
        <div className="max-w-4xl mx-auto px-8 pt-10 animate-slideUp">
          <button onClick={() => setActiveDepartment(null)} className="mb-8 flex items-center gap-2 text-slate-400 font-bold text-xs uppercase tracking-widest hover:text-indigo-600">
            <ArrowLeftIcon className="h-4 w-4" /> Back to Departments
          </button>
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <h2 className="text-3xl font-black text-slate-900 mb-8 tracking-tighter">Academic <span className="text-indigo-600">Batches</span></h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
              {availableBatches.map(b => (
                <div key={b} onClick={() => setSelectedBatch(b)} className="p-10 border border-slate-100 rounded-3xl text-center cursor-pointer hover:bg-slate-900 group transition-all shadow-sm">
                  <span className="block text-4xl font-black text-indigo-600 group-hover:text-white mb-2">{b}</span>
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Entry Batch</span>
                </div>
              ))}
            </div>
            {availableBatches.length === 0 && <p className="text-center text-slate-400 py-10">No batches found for this department.</p>}
          </div>
        </div>
      )}

      {/* 3. SEMESTER SELECTION */}
      {!isLoading && selectedBatch && !selectedSemester && (
        <div className="max-w-4xl mx-auto px-8 pt-10 animate-slideUp">
          <button onClick={() => setSelectedBatch(null)} className="mb-8 flex items-center gap-2 text-slate-400 font-bold text-xs uppercase tracking-widest hover:text-indigo-600">
            <ArrowLeftIcon className="h-4 w-4" /> Back to Batches
          </button>
          <div className="bg-white p-10 rounded-[2.5rem] border border-slate-200 shadow-sm">
            <h2 className="text-3xl font-black text-slate-900 mb-8 tracking-tighter">Academic <span className="text-indigo-600">Semesters</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {availableSemesters.map((sem) => (
                <div 
                  key={sem} 
                  onClick={() => setSelectedSemester(sem)}
                  className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl cursor-pointer hover:bg-indigo-600 group transition-all"
                >
                  <div className="flex items-center gap-4">
                    <CalendarDaysIcon className="h-6 w-6 text-indigo-600 group-hover:text-white" />
                    <span className="font-bold text-slate-800 group-hover:text-white">{sem}</span>
                  </div>
                  <ChevronRightIcon className="h-5 w-5 text-slate-300 group-hover:text-white" />
                </div>
              ))}
            </div>
          </div>
      ...
      {/* 4. FINAL STUDENT LIST */}
      {!isLoading && selectedSemester && (
        <div className="max-w-7xl mx-auto px-8 pt-10 animate-fadeIn">
          <div className="flex items-center justify-between mb-8">
            <button onClick={() => setSelectedSemester(null)} className="flex items-center gap-2 text-slate-400 font-bold text-xs uppercase tracking-widest hover:text-indigo-600">
              <ArrowLeftIcon className="h-4 w-4" /> Change Period
            </button>
            <CSVLink
              data={csvData}
              filename={`students_${selectedBatch}_${selectedSemester}.csv`}
              className="bg-emerald-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center gap-2 hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100"
            >
              <DocumentArrowDownIcon className="h-5 w-5" /> Export CSV
            </CSVLink>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded-[2rem] shadow-sm overflow-hidden">
            <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row justify-between items-center gap-4">
               <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tighter uppercase">{activeDepartment.department_name}</h2>
                  <p className="text-slate-500 text-xs font-bold uppercase tracking-widest">Batch {selectedBatch} • {selectedSemester}</p>
               </div>
               <div className="relative w-full md:w-80">
                  ...
                  <input
                    ...
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                    className="w-full pl-10 pr-4 py-3 bg-white border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500"
                  />
               </div>
            </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-white">
                  ...
                  <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">ID Number</th>
                  ...
                  <React.Fragment key={s.username}>
                    <tr className="hover:bg-indigo-50/30 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-slate-900 rounded-2xl flex items-center justify-center text-indigo-400 font-black text-lg">
                      ...
                      <td className="px-8 py-6 font-bold text-slate-500">{s.username}</td>
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2">
                          <button onClick={() => setViewingStudent(viewingStudent === s.username ? null : s.username)} className="p-3 text-indigo-600 hover:bg-indigo-100 rounded-xl transition-all">
                            <EyeIcon className="h-6 w-6" />
                          </button>
                          <button onClick={() => downloadSlip(s, selectedSemester)} className="p-3 text-slate-900 hover:bg-slate-200 rounded-xl transition-all">
                    ...
                    {viewingStudent === s.username && (
                      <tr>
                        <td colSpan={3} className="px-8 py-8 bg-slate-50">
                          <div className="bg-white rounded-3xl p-8 border border-slate-200 shadow-xl relative animate-slideDown">
                            <button onClick={() => setViewingStudent(null)} className="absolute top-6 right-6 p-2 hover:bg-slate-100 rounded-full"><XMarkIcon className="h-5 w-5" /></button>
                            <div className="grid md:grid-cols-3 gap-8">
                               <div>
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Full Name</label>
                                  <p className="font-bold text-slate-900">{s.user_details?.firstName} {s.user_details?.fatherName} {s.user_details?.grandFatherName}</p>
                               </div>
                               <div>
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Email Address</label>
                                  <p className="font-bold text-slate-900">{s.user_details?.email || 'N/A'}</p>
                               </div>
                               <div>
                                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Courses Enrolled</label>
                                  <div className="mt-2 space-y-1">
                                    {s.semesters[selectedSemester].map(c => (
                                      <div key={c.course_id} className="text-xs bg-indigo-50 text-indigo-700 px-3 py-1 rounded-lg flex justify-between">
                                        <span>{c.course_name}</span>
                                        <span className="font-bold">{c.course_credit}cr</span>
                                      </div>
                                    ))}
                                  </div>
                               </div>
