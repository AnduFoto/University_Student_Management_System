...
//     // Validation
//     if (!formData.username || !formData.department_id || formData.course_ids.length === 0) {
...
//   const handleDelete = async (studentUsername) => {
//     if (!window.confirm('Are you sure you want to delete this student record?')) return;

//     try {
//       const token = localStorage.getItem('access');
//       await axios.delete(
//         `${BASE_URL}/students/${studentUsername}/`,
//         {
//           headers: {
//             'Authorization': `Bearer ${token}`
//           }
...
//               <PlusIcon className="h-5 w-5 mr-2 text-blue-500" /> Register New Student
//             </h2>
//             <button
//               onClick={fetchData}
...
//                       {dept.department_name} - {getCollegeNameFromDepartment(dept.department_id)}
//                     </option>
//                   ))}
//                 </select>
//               </div>
...
//         {/* Students List - Fixed height with scroll */}
//         <div className="bg-white rounded-xl shadow-lg p-6 h-[calc(100vh-180px)] flex flex-col">
...
//                   className="pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent w-full transition-all"
//                 />
//               </div>
//             </div>
//           </div>
...
//                 <p className="text-sm mt-1">{searchTerm ? "Try adjusting your search" : "Register a student to get started"}</p>
//               </div>
//             ) : (
...
//                   return (
//                     <div key={student.username} className="border border-gray-200 rounded-lg overflow-hidden transition-all hover:shadow-md">
//                       <div 
...
//                             <div>
//                               <span className="font-medium text-gray-700">Year/Semester:</span>
//                               <span className="ml-2 text-gray-600">{student.year} Year, Sem {student.semester}</span>
//                             </div>
//                             <div>
//                               <span className="font-medium text-gray-700">Category:</span>
//                               <span className="ml-2 text-gray-600">{student.course_category || 'N/A'}</span>
//                             </div>
//                             <div className="col-span-2">
//                               <span className="font-medium text-gray-700">Courses:</span>
//                               <div className="mt-1 space-y-1">
//                                 {student.course_ids && student.course_ids.length > 0 ? (
//                                   student.course_ids.map(courseId => {
//                                     const course = getCourseDetails(courseId);
//                                     return course ? (
//                                       <div key={courseId} className="flex items-center text-gray-600">
//                                         <span className="w-2 h-2 bg-blue-500 rounded-full mr-2"></span>
//                                         {course.course_name} ({course.course_credit} credits)
//                                       </div>
//                                     ) : (
//                                       <div key={courseId} className="text-gray-400">Course ID: {courseId}</div>
//                                     );
//                                   })
//                                 ) : (
//                                   <span className="text-gray-400">No courses registered</span>
//                                 )}
//                               </div>
//                             </div>
//                           </div>
...
//   const [viewMode, setViewMode] = useState('department'); // 'department', 'students', 'registration'
//   const [selectedStudentForNewSemester, setSelectedStudentForNewSemester] = useState(null);

//   // Year and semester choices from Django model
//   const YEAR_CHOICES = [
...
//     // Validation
//     if (!formData.username || !formData.department_id || formData.course_ids.length === 0 || !formData.year || !formData.semester) {
//       setError('Please fill in all required fields');
//       setLoading(false);
//       return;
//     }
...
//     if (!selectedDepartment || !selectedYearSemester.year || !selectedYearSemester.semester) {
//       return [];
//     }
...
//           Select Year and Semester for {getDepartmentName(selectedDepartment)}
//         </h2>
//       </div>
...
//           </button>
//           <h2 className="text-xl font-semibold text-gray-800">
//             Students in {getDepartmentName(selectedDepartment)} - {selectedYearSemester.year} Year, Semester {selectedYearSemester.semester}
//           </h2>
//         </div>
...
//                 className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
//               >
//                 {COURSE_CATEGORY_CHOICES.map((category) => (
...
//         dept.department_id?.toLowerCase().includes(departmentSearch.toLowerCase())
//       );
...
//       const token = localStorage.getItem('access');
//       const response = await axios.get(
//         `${import.meta.env.VITE_API_BASE_URL}/students/check-existing/`,
//         {
//           headers: {
//             'Authorization': `Bearer ${token}`
//           },
...
//       // Fetch users
//       const usersResponse = await safeApiCall(`${import.meta.env.VITE_API_BASE_URL}/users/`);
//       const userList = usersResponse.results || [];
//       const studentUsers = userList.filter(user => user.role === 'student');
//       setUsers(studentUsers);
//       setFilteredUsers(studentUsers);
...
//       // Fetch colleges
//       const collegeResponse = await safeApiCall(`${import.meta.env.VITE_API_BASE_URL}/collages/colleges/`);
//       setColleges(collegeResponse.results || collegeResponse || []);

//       // Fetch departments
//       const deptResponse = await safeApiCall(`${import.meta.env.VITE_API_BASE_URL}/collages/departments/`);
//       setDepartments(deptResponse.results || deptResponse || []);
//       setFilteredDepartments(deptResponse.results || deptResponse || []);

//       // Fetch courses
//       const coursesResponse = await safeApiCall(`${import.meta.env.VITE_API_BASE_URL}/courses/courses/`);
//       setCourses(coursesResponse.results || coursesResponse || []);

//       // Fetch students
//       const studentsResponse = await safeApiCall(`${import.meta.env.VITE_API_BASE_URL}/students/`);
//       setStudents(studentsResponse.results || studentsResponse || []);
...
//       const response = await axios.post(
//         `${import.meta.env.VITE_API_BASE_URL}/students/create/`,
//         submissionData,
//         {
//           headers: {
//             'Authorization': `Bearer ${token}`,
...
//         `${import.meta.env.VITE_API_BASE_URL}/students/${studentUsername}/`,
//         {
//           headers: {
//             'Authorization': `Bearer ${token}`
//           },
...
//         setMessage('Student courses deleted successfully!');
//         // Refresh the students list to reflect the changes
//         fetchData();
//       } else {
//         setError('Failed to delete student courses');
//       }
//     } catch (err) {
//       setError('Failed to delete student courses. Please check your connection.');
//       console.error('Delete error:', err);
//     }
//   };
...
//             <div className="font-medium text-gray-900">{dept.department_name}</div>
//             <div className="text-sm text-gray-500">
//               ID: {dept.department_id} • {getCollegeNameFromDepartment(dept.department_id)}
//             </div>
//           </div>
//         ))}
//       </div>
...
//             {departmentSearch ? "Try adjusting your search" : "No departments available"}
//           </p>
//         </div>
//       )}
//     </div>
        ...
        <div className="flex-grow overflow-y-auto pr-2">
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-3 px-4 rounded-xl hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 transition-all duration-200 font-medium shadow-md hover:shadow-lg"
            >
              {loading ? (
                <span className="flex items-center justify-center">
                  <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-white mr-2"></div>
